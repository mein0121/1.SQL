...
-- TODO: EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 부서(dept_name)를 조회. 단 직원이름(emp_name)은 모두 대문자,
--       부서(dept_name)는 모두 소문자로 출력.
-- UPPER/LOWER
select  emp_id,
        upper(emp_name) "emp_name",
        salary,
        lower(dept_name) "dept name"
from   emp;        
...
--(단, "급여(salary)" 열을 길이가 7인 문자열로 만들고, 길이가 7이 안될 경우 왼쪽부터 빈 칸을 '_'로 채우시오. EX)__5000) -LPAD() 이용
select  emp_name,
        lpad(salary, 7, '-') salary,
        rpad(salary, 7) salary2,
        salary        
from    emp;        
...
select  emp_id,
        ...
        --salary - sal_raise --컬럼과 컬럼을 계산할때는 파생컬럼(파생변수)은 피연산자로 올수 없다.
from   emp;  
...
--TODO: 문자열 '20100107232215' 를 2010년 01월 07일 23시 22분 15초 로 출력. (dual 테이블 사용)
-- 문자열 -> date -> 문자열(원하는형식)
...
select *
from   emp
order by case job when 'ST_CLERK' then '10'
                  when 'IT_PROG' then '20'
                  when 'PU_CLERK' then '30'
                  when 'SA_MAN' then '40'
                  else job  end asc;
