...
select  round(avg(comm_pct), 2), --comm_pct가 있는 직원들의 comm_pct평균
        round(avg(nvl(comm_pct, 0)), 3) --comm_pct가 없을 경우 0으로 치환한뒤 평균계산. 전체직원의 평균
from    emp;
...
--where  length(emp_name) = max(length(emp_name));-- where 절에는 집계(그룹)함수를 사용못함.

--TODO: EMP 테이블의 부서(dept_name)가 몇종류가 있는지 조회. 
-- 고유값들의 개수
select  count(distinct dept_name),--null 은 빼고 셈
        count(distinct nvl(dept_name, 'a')) --null을 포함해서 셈. 
from emp;
...
select  decode(grouping_id(mgr_id), 1, '총계',  mgr_id) "상사ID", --decode()의 반환값으로 문자열과 다른 타입을 사용할 경우 문자열을 먼저 기술.
        count(*) "직원수"
from   emp
